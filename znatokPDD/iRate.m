...
- (BOOL)shouldPromptForRating {
    ...
    else if (self.ratedThisVersion) {    //check if we've rated this version
    ...
    else if (self.ratedAnyVersion && !self.promptForNewVersionIfUserRated) {    //check if we've rated any version
        if (self.verboseLogging) {
    ...
    else if (self.declinedAnyVersion) {    //check if we've declined to rate the app
        if (self.verboseLogging) {
    ...
    else if ((self.daysUntilPrompt > 0.0f || self.usesPerWeekForPrompt) && self.firstUsed == nil) {    //check for first launch
        if (self.verboseLogging) {
    ...
    else if ([[NSDate date] timeIntervalSinceDate:self.firstUsed] < self.daysUntilPrompt * SECONDS_IN_A_DAY) {    //check how long we've been using this version
        if (self.verboseLogging) {
    ...
    else if (self.usesCount < self.usesUntilPrompt && self.eventCount < self.eventsUntilPrompt) {    //check how many times we've used it and the number of significant events
        if (self.verboseLogging) {
    ...
    else if (self.usesPerWeek < self.usesPerWeekForPrompt) { //check if usage frequency is high enough
        if (self.verboseLogging) {
    ...
    else if (self.lastReminded != nil && [[NSDate date] timeIntervalSinceDate:self.lastReminded] < self.remindPeriod * SECONDS_IN_A_DAY) {    //check if within the reminder period
        if (self.verboseLogging) {
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    else if (([self.cancelButtonLabel length] && buttonIndex == 2) || ([self.cancelButtonLabel length] == 0 && buttonIndex == 1)) {
        [self remindLater];
    }
    else {
        [self rate];
