...
- (void)viewDidLoad {
    [super viewDidLoad];
    NSString *minutes = [NSString stringWithFormat:@"%lu", _time / 60];
    NSString *seconds = [NSString stringWithFormat:@"%lu", _time % 60];
    NSUInteger myMinute = [minutes intValue];
    NSUInteger mySecond = [seconds intValue];
    if (myMinute < 10)
        minutes = [NSString stringWithFormat:@"0%lu", _time / 60];
    if (mySecond < 10)
        seconds = [NSString stringWithFormat:@"0%lu", _time % 60];
    ...
    if (_rightCount > 17) {
        ...
        if (screenHeight == 480) {
            ...
            [resultImage.layer setBorderColor:[[UIColor redColor] CGColor]];
            [resultImage.layer setBorderWidth:1.0];
            [self.view addSubview:resultImage];
            UILabel  *resultLabel = [[UILabel alloc] initWithFrame:CGRectMake(100, 166, 118, 22)];
